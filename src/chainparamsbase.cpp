...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-hardforktime", "HARDFORK-PLUMBING: activate the fork's consensus rules for blocks whose own timestamp reaches this value (custom signet only, must have -signetchallenge set). Note this does not enter the network magic, so nodes on the same custom signet with different values will peer normally and then split at the first retarget after activation", ArgsManager::ALLOW_ANY | ArgsManager::DISALLOW_NEGATION | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
