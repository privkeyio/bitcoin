...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-extendedcoinbasematurity=<start>[:<end>]", strprintf("Schedule the extended coinbase maturity soft fork: coinbase outputs created from the first block whose parent's median-time-past reaches <start> cannot be spent until the median-time-past has advanced %d seconds past their block's, until <end> (default: the RDTS expiry, which neither may outlast). Requires -rdtsexpiry, which <start> must precede. (regtest-only)", EXTENDED_COINBASE_MATURITY_TIME), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
